<x-layouts.app>
    @if(session('success'))
        <div id="flash-success"
             class="fixed top-5 right-5 z-50 flex items-center gap-3 bg-green-600 text-white px-5 py-3 rounded-xl shadow-lg text-sm font-medium">
    ...
    <div class="p-6 md:p-8 max-w-full">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
            <div>
                ...
                <p class="text-sm text-slate-500 mt-0.5">Gerencie seus clientes e seus acessos de sistemas.</p>
            </div>
        ...
        {{-- Search --}}
        <form method="GET" action="{{ route('clientes.index') }}" class="flex flex-wrap items-center gap-3 mb-6">
            ...
            @if($search)
                <a href="{{ route('clientes.index') }}" class="text-sm text-blue-600 hover:underline">Limpar</a>
            @endif
        </form>
        ...
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
            @if($clientes->isEmpty())
                <div class="flex flex-col items-center justify-center py-20 text-center">
                    ...
                    <p class="font-semibold text-slate-700">Nenhum cliente cadastrado</p>
                    <p class="text-sm text-slate-400 mt-1">Cadastre seu primeiro cliente clicando em "Novo Cliente".</p>
                </div>
            @else
                    ...
                    <table class="w-full text-sm">
                        <thead>
                            <tr class="border-b border-slate-100 bg-slate-50/60">
                                <th class="text-left font-semibold text-slate-500 px-6 py-3.5">Nome</th>
                                <th class="text-left font-semibold text-slate-500 px-4 py-3.5">E-mail</th>
                                <th class="text-left font-semibold text-slate-500 px-4 py-3.5">Telefone</th>
                                <th class="text-center font-semibold text-slate-500 px-4 py-3.5">Propostas</th>
                                <th class="px-4 py-3.5 w-52"></th>
                            </tr>
                        </thead>
                            ...
                            @foreach($clientes as $cliente)
                                <tr class="hover:bg-slate-50/40 transition-colors group">
                                    ...
                                    <td class="px-4 py-4 text-center">
                                        @if($cliente->propostas_count > 0)
                                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-lg text-xs font-semibold bg-blue-100 text-blue-700">
                                                {{ $cliente->propostas_count }}
                                            </span>
                                        @else
        ...
        <p class="text-xs text-slate-400 mt-3">
            Total: {{ $clientes->total() }} cliente{{ $clientes->total() !== 1 ? 's' : '' }}
        </p>
    </div>
    ...
    <div id="delete-modal" class="fixed inset-0 z-50 hidden items-center justify-center">
        ...
        <div class="relative bg-white rounded-2xl shadow-xl p-6 mx-4 w-full max-w-sm animate-scale-in">
            <div class="flex items-center gap-3 mb-4">
                ...
                <div>
                    <h3 class="font-bold text-slate-900">Excluir cliente</h3>
                    <p class="text-sm text-slate-500">Esta ação não pode ser desfeita.</p>
                </div>
            </div>
            <p class="text-sm text-slate-600 mb-5">
                Tem certeza que deseja excluir o cliente <strong id="delete-modal-nome" class="text-slate-900"></strong>?
            </p>
