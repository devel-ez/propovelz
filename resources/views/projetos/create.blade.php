<x-layouts.app>
    <div class="p-6 md:p-8 max-w-2xl">
        ...
        <div class="flex items-center gap-3 mb-8">
            ...
            <div>
                <h1 class="text-2xl font-bold text-slate-900">Novo Projeto</h1>
                <p class="text-sm text-slate-500 mt-0.5">Preencha os dados do projeto.</p>
            </div>
        </div>
        ...
        @if($clientes->isEmpty())
            <div class="bg-amber-50 border border-amber-200 rounded-2xl p-5 flex items-start gap-3">
                ...
                <div>
                    ...
                    <p class="text-sm text-amber-700 mt-0.5">Cadastre um cliente primeiro antes de criar um projeto.</p>
                </div>
            </div>
        @else
            <form action="{{ route('projetos.store') }}" method="POST" class="space-y-5">
                @csrf

                @include('projetos._form', ['projeto' => null, 'clientes' => $clientes, 'propostas' => $propostas])
